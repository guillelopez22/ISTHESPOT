  ...
  <div id="root">
    ...
    <div class="row">
        <div class="input-field col s6">
          <input v-on:input="insumo.nombre = $event.target.value" type="text" v-model="insumo.nombre" :disabled="loading"  id="Nombre">
          <label for="Nombre">Nombre</label>
        </div>
        <div class="input-field col s6">
          <input v-on:input="insumo.inventario = $event.target.value" type="number" v-model="insumo.inventario" :disabled="loading"  id="Inventario">
  ...
  methods: {
      ...
      borrarProveedor(){
        ...
        this.insumo.idProveedor = this.idProv;
				this.$http.post('http://localhost:8000/insumos/create',this.insumo)
				.then((response)=>{
					this.loading=false;
      ...
      modifyInsumo(){
        ...
        if(this.idModificar!=''){
          Materialize.updateTextFields();
          this.insumo.idProveedor = this.idProv;
          this.$http.put('http://localhost:8000/insumos/update/'+this.idModificar,this.insumo)
  				.then((response)=>{
  					if(response.body.success){
              this.getInsumo();
              this.loading=false;
      ...
      deleteInsumo(idInsumo){
          this.loading=true;
          this.$http.delete('http://localhost:8000/insumos/delete/'+idInsumo)
            .then((response)=>{
            this.loading=false;
            if(response.body.success){
              this.getInsumo();
